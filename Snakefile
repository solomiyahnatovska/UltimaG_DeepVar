...
rule make_intervals:
    ...
    shell:
        "/home/hnatovs1/scratch/scripts_eDV/make_intervals.sh -i {input.interval_list} -o {output.beddir} &> {log}"
rule make_examples:
    input:
    ...
    shell:
        "/home/hnatovs1/scratch/scripts_eDV/make_examples.sh -c {input.cram_file} -d {input.cram_index} -i {input.beddir} -o {output.examplesdir} &> {log}"
